...
def fp_decoupled(comm, experiment, args, postprocess=None):
    """
    Method by Lasarzik, R., & Reiter, M. (2023). Analysis and numerical approximation of energy-variational solutions to the Ericksen-Leslie equations. Acta Appl. Math., 184, 44.
    ...
    vl0, pl0, dl0, ql0                  = Function(P2),Function(P1), Function(D), Function(Y) # for inner fp iteration
    ...
    a1, a2, a3, L1, L2, L3 = variational_forms(dxL, v1, p1, d1, q1, vl0, pl0, dl0, ql0, v0, p0, d0, q0, a_test, h_test, b_test, c_test, grad_d0, dt, n_F, args, H=H, f= None,  p_dirichlet_bcs_exist = p_dirichlet_bcs_exist)
    #!SECTION
    ...
    metrics = compute_metrics(comm, args, v0,p0,d0,q0,  d0, d0, dxL , H=H) # for initial condition
    if postprocess and comm.rank == 0:
    ...
    while t < args.T:
        ...
        while not (fp_err_v <= np.maximum(args.fp_a_tol, args.fp_r_tol * val_v ) and fp_err_d<= np.maximum(args.fp_a_tol, args.fp_r_tol * val_d )) and (fp_iter < args.fp_max_iters):
            ...
            A1 = assemble_matrix_nest(a1, bcs=bcs_vp)
            A1.assemble()
            b1 = assemble_vector_nest(L1)
            apply_lifting_nest(b1, a1, bcs=bcs_vp)
            for b_sub in b1.getNestSubVecs():
                b_sub.ghostUpdate(addv=PETSc.InsertMode.ADD, mode=PETSc.ScatterMode.REVERSE)
            bcs0 = bcs_by_block(extract_function_spaces(L1), bcs_vp)
            set_bc_nest(b1, bcs0)          
            ...
            if not p_dirichlet_bcs_exist:
                ...
                # solution process.
                null_vec = create_vector_nest(L1)
            ...
            ksp1 = setup_solver_NSE(comm, args, A1, p1, h_test)
            ...
            fp_err_v, val_v = compute_fp_metrics_v(comm, args, vl, vl0)  
            ...
            #SECTION - METRICS            
            metrics = compute_metrics(comm, args, vl,pl,dl,ql, d0, 0.5*(dl+d0), dxL, H=H)
...
def variational_forms(dxL, v1, p1, d1, q1, vl0, pl0, dl0, ql0, v0, p0, d0, q0, a, h, b, c, grad_d0, dt, normal_F, args, H=None, f= None, p_dirichlet_bcs_exist = False):
        """
        ...
        L_1 -= dt*inner(dot(grad_d0 , a), a_times_b_times_c(dl0_,dl0_, ql0))*dxL

        # Leslie stress tensor
        if args.beta != 0.0:
            L_1 += dt* args.beta *inner(dot(grad_skw(a),dl0_), a_times_b_times_c(dl0_,dl0_,ql0))*dxL
        if args.lam != 0.0:
            L_1 -= dt* args.lam *inner(dot(grad_sym(a),dl0_), a_times_b_times_c(dl0_,dl0_,ql0))*dxL
        ...
        # Ericksen stress tensor
        a_33 -= 0.5*dt*inner(dot(grad_d0 , vl0), a_times_b_times_c(dl0_,d1, c))*dxL
        L_3  += 0.5*dt*inner(dot(grad_d0 , vl0), a_times_b_times_c(dl0_,d0, c))*dxL

        # Leslie stress tensor
        if args.beta != 0.0:
            a_33 += 0.5*dt* args.beta *inner(dot(grad_skw(vl0),dl0_), a_times_b_times_c(dl0_,d1,c))*dxL
            L_3  -= 0.5*dt* args.beta *inner(dot(grad_skw(vl0),dl0_), a_times_b_times_c(dl0_,d0,c))*dxL
        if args.lam != 0.0:
            a_33 -= 0.5*dt* args.lam *inner(dot(grad_sym(vl0),dl0_), a_times_b_times_c(dl0_,d1,c))*dxL           
            L_3  += 0.5*dt* args.lam *inner(dot(grad_sym(vl0),dl0_), a_times_b_times_c(dl0_,d0,c))*dxL
        ...
        return a, form(a_33), form(a_44), L, form(L_3), form(L_4)
...
def setup_solver_NSE(comm, args, A, p1, h):
...
def compute_fp_metrics_v(comm, args, vl, vl_):
    e_v = vl - vl_
...
def compute_metrics(comm, args, v,p,d,q, d0, d_, dxL, H=None, id =""):
    # ENERGY TERMS
