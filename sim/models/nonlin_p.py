...
def nonlin_p(comm, experiment, args, postprocess=None):
    """
    Penalization Method based on Algorithm 3.1 by Becker, R., Feng, X., & Prohl, A. (2008). Finite element approximations of the Ericksen-Leslie model for nematic liquid crystal flow. SIAM Journal on Numerical Analysis, 46(4), 1704-1731. doi:10.1137/07068254X
    ...
    if postprocess and comm.rank == 0:
        postprocess.log("dict", "static",{"SID": args.sim_id, "MODEL":args.mod, "MASS LUMPING" : args.mass_lumping, "EPS": args.alpha, "EXPERIMENT":args.exp, "mesh res.": args.dh, "dt":args.dt, "dim":args.dim, "T": args.T, "ksp": args.ksp_type_n, "pc" : args.pc_type_n})
    ...
    problem0 = LinearProblem(form(inner(qc1, bc_test)*dx), form(q_elastic_energy(args, dxL, d0, d0, bc_test, H = None)),  bcs=[], u=q_ic, petsc_options={"ksp_type": "preonly", "pc_type": "lu", "pc_factor_mat_solver_type": "mumps"})
    problem0.solve()
    ...
    a, L = variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, a, h, b, c, dt, n_F, args, H=H, f= None, p_dirichlet_bcs_exist = p_dirichlet_bcs_exist)
...
def variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, a, h, b, c, dt, normal_F, args, H=None, f= None, p_dirichlet_bcs_exist = False):
        """
        ...
        a_43 = q_elastic_energy(args, dxL, d1, d0, b, H = H)
...
def q_elastic_energy(args, dxL, d1, d0, b, H = None):
    eps = args.alpha # Penalization Parameter

    eq = args.K1 * inner( grad(d1), grad(b))*dx
    # variational derivative of the double well potential for the relaxation of the unit-sphere constraint
    # d_      = 0.5*d1 + 0.5*d0                           # midpoint discretization
    # d_norm_ = 0.5*inner(d1,d1)+0.5*inner(d0,d0)         # midpoint discretization [|d|^2]^{j-1/2}
    # eq += 1/eps*(d_norm_-1)*inner(d_, b)*dxL
    # based on the formulation by Becker, Feng and Prohl
    eq += 1/eps*(inner(d1,d1)*inner(d1,b) - inner(d0,b))*dxL # NOTE - this is not exactly the same as the midpoint discretization but has the advantage that it only depends on the current iterate and thus can be implemented more easily in this framework. Since we are interested in the limit eps --> 0, the difference should be negligible anyways.
...
def compute_metrics(comm, args, u1, u0, dxL, H= None, id =""):
    ...
    dissipation_form += dt* args.gamma *inner(q, q)*dx
