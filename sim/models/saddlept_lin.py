...
def saddlept_lin(comm, experiment, args, postprocess=None):
    """
    Semi-implicit method (see Equations 27 and 15) by Badia, S., Guillen-González, F., & Gutiérrez-Santacreu, J. (2011). Finite element approximation of nematic liquid crystal flows using a saddle-point structure. J. Comput. Phys., 230(4), 1686-1706.
    ...
    if postprocess and comm.rank == 0:
        postprocess.log("dict", "static",{"SID": args.sim_id, "MODEL":args.mod, "EPSILON/ALPHA": args.alpha, "MASS LUMPING" : args.mass_lumping, "EXPERIMENT":args.exp, "mesh res.": args.dh, "dt":args.dt, "dim":args.dim, "T": args.T})
    ...
    FS_L        = functionspace(domain, ElementMetaData("Lagrange", 1 )) # space for the scalar Lagrangian multiplier
    TensorF     = functionspace(domain, ElementMetaData("Lagrange", 1 , shape=(dim, dim)))
    
    v1,p1, d1                           = TrialFunction(P2),TrialFunction(P1), TrialFunction(D)
    v0,p0, d0, d_                       = Function(P2),Function(P1), Function(D), Function(D) # d_ is used for the tangent projection
    v,p, d                              = Function(P2),Function(P1), Function(D)
    a_test, h_test, c_test              = TestFunction(P2),TestFunction(P1), TestFunction(D)
    grad_d0_project                     = Function(TensorF)  

    # For the Lagrangian Multiplier
    xi1 = TrialFunction(FS_L)
    psi = TestFunction(FS_L) 
    xi  = Function(FS_L)
    xi0 = Function(FS_L)
    ...
    local_dofs = P2.dofmap.index_map.size_local + P1.dofmap.index_map.size_local + D.dofmap.index_map.size_local + FS_L.dofmap.index_map.size_local    # Count DOFs on this rank    
    ...
    a, L = variational_form(dxL, v1, p1, d1, xi1, v0, p0, d0, xi0, a_test, h_test, c_test, psi, grad_d0, d_,  dt, n_F, args, H=H, f= None, p_dirichlet_bcs_exist = p_dirichlet_bcs_exist)
    #!SECTION
    ...
    metrics = compute_metrics(comm, args, v0,p0,d0,d0, dxL , H = H) # for initial condition
    if postprocess and comm.rank == 0:
    ...
    while t < args.T:
        ...
        x = PETSc.Vec().createNest([la.create_petsc_vector_wrap(v.x), la.create_petsc_vector_wrap(p.x), la.create_petsc_vector_wrap(d.x), la.create_petsc_vector_wrap(xi.x)])
        ...
        #SECTION - UPDATE
        update_and_scatter([v0,p0,d0,d_,xi0], [v,p,d,d,xi])
        ...
        metrics =  compute_metrics(comm, args, v,p,d,d0, dxL , H=H)
        total_time = mpi_time(comm, start = total_time_start )
        if postprocess and comm.rank == 0:
...
def variational_form(dxL, v1, p1, d1, xi1, v0, p0, d0, xi0, a, h, c, psi, grad_d0, d_, dt, normal_F, args, H=None, f= None, p_dirichlet_bcs_exist = False):
        """
        ...
        # a_14 = (-1)*dt*inner(dot(grad_d0 , a), q1)*dxL
        a_13 = (-1)*dt*q_elastic_energy(args, d1, d0, dot(grad_d0 , a), H = H)

        # Leslie stress tensor
        if args.beta != 0.0:
            # a_14 += dt* args.beta *inner(dot(grad_skw(a),d_), q1)*dxL
            a_13 += dt* args.beta *q_elastic_energy(args, d1, d0, dot(grad_skw(a),d_), H = H)
        if args.lam != 0.0:
            # a_14 -= dt* args.lam *inner(dot(grad_sym(a),d_), q1)*dxL
            a_13 -= dt* args.lam *q_elastic_energy(args, d1, d0, dot(grad_sym(a),d_), H = H)
        ...
        # Ericksen stress tensor
        a_31 = dt*inner(dot(grad_d0 , v1), c)*dx
        ...
        if args.gamma != 0.0:
            # NOTE - this time the elastic energy can be included explicitly
            a_33 = dt* args.gamma * q_elastic_energy(args, d1, d0, c, H = H)
        ...
        a_43 = psi*inner(d0,d1)*dxL

        a_44 = (-1)*(args.alpha**2)*0.5* xi1 * psi *dxL

        L_4 = psi*inner(d0,d0)*dxL - (args.alpha**2)*0.5* xi0 * psi *dxL
...
def compute_metrics(comm, args, v,p,d,d0, dxL, H= None, id =""):
    # ENERGY TERMS
