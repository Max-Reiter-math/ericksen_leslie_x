...
def fp_linear(comm, experiment, args, postprocess=None):
    """
    ...
    if postprocess and comm.rank == 0:
        postprocess.log("dict", "static",{"SID": args.sim_id, "MODEL":args.mod, "PROJECT TANGENT MATRIX": args.project_tangent_map, "MASS LUMPING" : args.mass_lumping, "EXPERIMENT":args.exp, "mesh res.": args.dh, "dt":args.dt, "dim":args.dim, "T": args.T, "fp_a_tol" : args.fp_a_tol, "fp_r_tol" : args.fp_r_tol, "fp_max_iters" : args.fp_max_iters})
    ...
    a, L = variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, a_test, h_test, b_test, c_test, grad_d0, d_, dl0,  dt, n_F, args, H=H, f= None, p_dirichlet_bcs_exist = p_dirichlet_bcs_exist)
    #!SECTION
    ...
    while t < args.T:
        ...
        while not (fp_err_v <= np.maximum(args.fp_a_tol, args.fp_r_tol * val_v ) and fp_err_d<= np.maximum(args.fp_a_tol, args.fp_r_tol * val_d )) and (fp_iter < args.fp_max_iters):
            ...
            metrics = compute_metrics(comm, args, vl,pl,dl,d0,ql, dxL , H=H)
        ...
        #SECTION - UPDATE
        update_and_scatter([v0,p0,d0,d_,q0], [vl,pl,dl,dl,ql])
        if args.mass_lumping: 
            grad_d0_project.interpolate(project_lumped(grad(d0),TensorF))
            grad_d0_project.x.scatter_forward()
        #!SECTION 
...
def variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, a, h, b, c, grad_d0, d_, dl0, dt, normal_F, args, H=None, f= None, p_dirichlet_bcs_exist = False):
        """
