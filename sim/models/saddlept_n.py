from ufl import grad, div, curl, split, inner, dot, cross, dx, Measure, FacetNormal, TestFunction
from basix.ufl import element, mixed_element
from dolfinx import log
from dolfinx.fem import Function, functionspace, form, assemble_scalar
from dolfinx.fem.petsc import assemble_matrix, NonlinearProblem
from dolfinx.nls.petsc import NewtonSolver
from sim.common.operators import *
from sim.common.common_fem_methods import *
from sim.common.meta_bcs import *

def saddlept_n(comm, experiment, args, postprocess=None):
    # NOTE - not performing well currently..
    """
    Implicit method (see Equations 20 and 15) using an exact Newton-scheme by Badia, S., Guillen-González, F., & Gutiérrez-Santacreu, J. (2011). Finite element approximation of nematic liquid crystal flows using a saddle-point structure. J. Comput. Phys., 230(4), 1686-1706.
    ...
    if postprocess and comm.rank == 0:
        postprocess.log("dict", "static",{"SID": args.sim_id, "MODEL":args.mod, "EPSILON/ALPHA": args.alpha, "MASS LUMPING" : args.mass_lumping, "EXPERIMENT":args.exp, "mesh res.": args.dh, "dt":args.dt, "dim":args.dim, "T": args.T, "max_it" : args.n_max_it, "ksp": args.ksp_type_n, "pc" : args.pc_type_n})
    ...
    me          = mixed_element([vP2, P1, vP1, P1])
    FS           = functionspace(domain, me)
    
    u1              = Function(FS)
    v1,p1, d1, xi1  = split(u1)
    u0              = Function(FS)               # initial conditions
    v0,p0, d0, xi0  = split(u0)
    u_test          = TestFunction(FS)
    a,h, c, psi     = split(u_test)
    ...
    problem_nonlin = NonlinearProblem(a1+a2 - L1 -L2, u1, bcs=bcs)
    #!SECTION
    ...
    if postprocess:
        v_out, p_out, d_out, xi_out = u0.sub(0).collapse(), u0.sub(1).collapse(), u0.sub(2).collapse(), u0.sub(3).collapse()
    ...
    while t < args.T:
        ...
        if postprocess and comm.rank == 0:
            ...
            postprocess.log("dict", t, {"time":t,"n_iters" : newton_iterations, "converged" :converged , "t.sol_ass": measure_time}, visible = True)  
