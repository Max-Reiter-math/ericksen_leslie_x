...
def linear_cg_bdf2(comm, experiment, args, postprocess=None):
    """
    BDF2 discretization of Algorithm 1 by Maximilian E. V. Reiter. (2025). Projection Methods in the Context of Nematic Crystal Flow.
    ...
    v00, d00                            = Function(P2), Function(D) # d_ is used for the tangent projection
    ...
    a, L = variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, v00, d00, a_test, h_test, b_test, c_test, grad_d0, d_,  dt, n_F, args, H=H, f= None, p_dirichlet_bcs_exist = p_dirichlet_bcs_exist)
    #!SECTION
    ...
    for iter in range(1,int(args.T/dt)+1,1):
        t = dt*iter 
        ...
        #SECTION - UPDATE
        d_.x.array[:] = 2*d.x.array[:] - d0.x.array[:] # BDF2 discretization
        d_.x.scatter_forward()
        
        if iter>=2: update_and_scatter([v00,d00], [v0,d0])
        update_and_scatter([v0,p0,d0,q0], [v,p,d,q])
        if args.mass_lumping: 
            grad_d0_project.interpolate(project_lumped(grad(d0),TensorF))
            grad_d0_project.x.scatter_forward()
        #!SECTION 
...
def variational_form(dxL, v1, p1, d1, q1, v0, p0, d0, q0, v00, d00, a, h, b, c, grad_d0, d_, dt, normal_F, args, H=None, f= None, p_dirichlet_bcs_exist = False):
        """
        ...
        L_1 = 2*inner(v0, a )*dx - 1/2*inner(v00, a )*dx # discrete time derivative (BFD2)
        ...
        L_3 = 2*inner(d0, c)*dxL - 1/2*inner(d00, c)*dxL # discrete time derivative (BFD2)
